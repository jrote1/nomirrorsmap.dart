...
class NoMirrorsMapStore {
  ...
  static Function getPropertySetter(String name) {
    return _propertyMappings.firstWhere((property) => property.propertyName == name).setter;
  }
  ...
  static Function getPropertyGetter(String name) {
    return _propertyMappings.firstWhere((property) => property.propertyName == name).getter;
  }
}
...
class ClassConverter implements Converter {
  ...
  dynamic _fromBaseObjectData(BaseObjectData baseObjectData, Type type) {
    if (baseObjectData is ClassObjectData) {
      ...
      if (baseObjectData.previousHashCode != null && instances.containsKey(baseObjectData.previousHashCode)) classConverterInstance =
      ...
      if (!classConverterInstance.filled && baseObjectData.properties.length > 0) {
        for (var property in generatedMap.properties) {
          if (baseObjectData.properties.containsKey(property.property.propertyName)) {
            ...
            Object value;
            if (converters.containsKey(propertyType)) value = converters[propertyType].to(propertyObjectData);
